...
main = do
    ...
    putStrLn $ unlines $ concatMap unParseChunk chunks


unParseChunk (NonRecordChunk ls) = ls
unParseChunk (RecordChunk r) = "\\begin{code}" : [LHEP.prettyPrint decl | decl <- recordToDecls r] ++ ["", "\\end{code}"]


data Chunk = NonRecordChunk [String] | RecordChunk Record
           deriving (Show)
...
identifyPresenceFields :: [Field] -> [(String, String)]
identifyPresenceFields [] = []
identifyPresenceFields (field:fields) = undefined


recordToDecls :: Record -> [Decl]
recordToDecls (Record { record_name, record_fields }) = [
    DataDecl noSrcLoc DataType [] (Ident record_name) [] [datacon] [],
    PatBind noSrcLoc (PVar (Ident $ "get" ++ record_name)) Nothing (UnGuardedRhs getexpr) (BDecls [])
  ]
  where
...
fieldToSyntax :: (String -> String) -> Field -> (Stmt, Maybe LHE.Type)
